...
def stringify_dict(value, depth, space_count=2):
    ...
    for key, val in value.items():
        lines.append(f'{deep_tab}   {key}: {stringify_dict(val, deep_size + 2)}')
        ...
        for k in data:
            if data[k][0] == 'added':
                if isinstance(data[k][1], dict):
                    lines.append(
                        f'{current_tab} + {k}: {stringify_dict(data[k][1], deep_size)}'
                    )
                else:
                ...
                if isinstance(data[k][1], dict) and not isinstance(data[k][2], dict):
                    lines.append(
                        f'{current_tab} - {k}: {stringify_dict(data[k][1], deep_size)}')
                    lines.append(f'{current_tab} + {k}: {data[k][2]}')
                elif not isinstance(data[k][1], dict) and isinstance(data[k][2], dict):
                    lines.append(f'{current_tab} - {k}: {data[k][1]}')
                    lines.append(
                        f'{current_tab} + {k}: {stringify_dict(data[k][1], deep_size)}'
                    )
                elif isinstance(data[k][1], dict) and isinstance(data[k][2], dict):
                    lines.append(
                        f'{current_tab} - {k}: {stringify_dict(data[k][1], deep_size)}')
                    lines.append(
                        f'{current_tab} + {k}: {stringify_dict(data[k][1], deep_size)}'
                ...
                lines.append(f'{current_tab}   {k}: {iter_(data[k][1], deep_size + 2)}')
